...
// Everything procedural (worldgen, belt clusters, debris rolls, art) must go through here so a
// given seed always produces the exact same result.
//
// API: mulberry32(seed) -> () => [0,1), hashString(str), hash2(x, y, seed), normalizeSeed(seed),
//      createRng(seed) -> { seed, next, float, int, chance, pick, sign, weighted, shuffle, fork(label) },
//      createNoise2D(seed) -> (x, y) => [0,1), fbm2D(noise, x, y, octaves, lacunarity, gain)
